...
const categorizeBox = (length, width, height, mass) => {
  const dimensionLimit = Math.pow(10, 4);
  const volumeLimit = Math.pow(10, 9);

  const bulky =
    length >= dimensionLimit ||
    width >= dimensionLimit ||
    height >= dimensionLimit ||
    mass >= dimensionLimit ||
    length * width * height >= volumeLimit;
...
console.log(categorizeBox(1000000, 5, 5, 101)); // "Both"
